...
class WaypointStepInput:
    ...
    Attributes:
        ...
        seed: The frame the world starts from, uint8 ``(H, W, 3)``, or ``None``
            when the application has no seed to offer.
        seed_id: Identifies the seed. A step whose ``seed_id`` differs from the
            one the model last applied starts a new world from ``seed``.
    """
...
class WaypointModel:
    ...
    def generate(self, step: WaypointStepInput) -> WaypointStepResult:
        ...
        world's first frame. Then generates four frames from the controls.
        ...
        index = self.index
        self.index += 1
        return WaypointStepResult(frames=frames, index=index)
