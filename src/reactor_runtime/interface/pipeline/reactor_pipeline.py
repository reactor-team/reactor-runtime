...
It owns one thing on top of :class:`ReactorApp`: the ``run()`` driver. It waits
on the live gate, advances the generator one ``yield`` at a time under the step
lock, emits each :class:`Output`, skips a turn on :data:`Idle`, and tears the
generator down cleanly when the gate drops. Because every ``@event`` handler
and lifecycle hook takes the same lock, a handler runs only between two
``yield`` points and ``self.state`` is consistent within one inference turn.

The typed state, its ``set_<field>`` commands, the step lock, and the live gate
come from :class:`ReactorApp`. A pipeline must declare ``state: MyState``; the
base leaves it optional.
"""
