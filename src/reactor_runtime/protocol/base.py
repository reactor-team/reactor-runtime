...
class Codec(ABC):
    ...
    def encode_session_ended(self, *, reason: str, message: str) -> tuple[Channel, bytes | str]:
        """Encode the platform's session-ended notice for the client.

        Like a moderation verdict, the notice is runtime-authored and
        unsolicited, so it rides a ``ControlServerMessage`` notification with no
        ``request_id``. *reason* is the stable token a client branches on;
        *message* is the human-readable sentence rendered for it. The physical
        channel is version-dependent and returned alongside the frame.
        """
        wire = control_pb2.ControlServerMessage(
            kind=common_pb2.MessageKind.MESSAGE_KIND_NOTIFICATION,
            session_ended=platform_pb2.SessionEnded(reason=reason, message=message),
        )
        return self.encode(wire)
