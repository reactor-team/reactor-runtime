...
Command handlers and lifecycle hooks run between steps, never during one. A loop that is not one step per emit overrides `run()` and keeps everything else: the state, the commands, the tracks, `emit()`, and `send()`.
