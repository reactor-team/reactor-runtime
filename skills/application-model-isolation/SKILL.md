...
Keep the two kinds of code in two places. The worked example is
...
`result.index` to tag the frames and to report progress; it never reads
`self.engine.index`.
...
class WaypointState(InputState):
    image: UploadedFile = InputField(default=None, description="...")
...
in the example decodes the upload before it becomes the seed, and `reset`
restarts the world from the seed it already has. Private fields (leading
underscore) are session scratch the application owns, such as the fitted seed
and its id.
