...
def test_detail_defaults_to_empty() -> None:
    t = Transition(SessionEvent.IDLING, SessionState.STREAMING, SessionState.STREAMING)
    assert t.detail == {}
...
def test_session_start_is_only_ready_to_waiting() -> None:
    ...
    rejoin = Transition(
        SessionEvent.CLIENT_CONNECTED, SessionState.ORPHANED, SessionState.STREAMING
    )
