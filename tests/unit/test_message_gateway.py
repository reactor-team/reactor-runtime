...
def _gateway(codec: Codec) -> tuple[MessageGateway, FakeSink, list[InboundCommand]]:
    sink = FakeSink()
    ...
    return MessageGateway(sink=sink, codec=codec, on_command=on_command), sink, received


async def test_ping_routes_to_keepalive() -> None:
    gateway, sink, received = _gateway(V1Codec())
    _, frame = V1Codec().encode(control_pb2.ControlClientMessage(ping=platform_pb2.Ping()))
    await gateway.handle(ConnId(7), frame, Channel.CONTROL)
...
async def test_command_is_decoded_and_emitted() -> None:
    ...
    await gateway.handle(ConnId(3), frame, Channel.DATA)
...
async def test_missing_request_id_is_minted() -> None:
    gateway, _, received = _gateway(V1Codec())
    _, frame = V1Codec().encode(data_pb2.DataClientMessage(command=model_pb2.Command(type="go")))
    await gateway.handle(ConnId(1), frame, Channel.DATA)
...
async def test_other_control_messages_are_not_routed() -> None:
    ...
    await gateway.handle(ConnId(1), frame, Channel.CONTROL)
    assert sink.keepalives == []
    assert received == []
...
async def test_undecodable_v1_frame_is_dropped() -> None:
    gateway, sink, received = _gateway(V1Codec())
    await gateway.handle(ConnId(1), b"\xff\xff\xff not protobuf", Channel.DATA)
    assert sink.keepalives == []
    assert received == []


async def test_malformed_v0_json_is_dropped() -> None:
    gateway, sink, received = _gateway(V0Codec())
    await gateway.handle(ConnId(1), "{ not json", Channel.DATA)
    assert sink.keepalives == []
    assert received == []


async def test_decodes_legacy_v0_command_the_same_way() -> None:
    gateway, _, received = _gateway(V0Codec())
    message = data_pb2.DataClientMessage(
        command=model_pb2.Command(type="spawn", data=dict_to_struct({"prompt": "hi"}))
    )
    channel, frame = V0Codec().encode(message)
    await gateway.handle(ConnId(2), frame, channel)
...
async def test_decodes_legacy_v0_ping_routed_by_message_type() -> None:
    gateway, sink, _ = _gateway(V0Codec())
    ...
    await gateway.handle(ConnId(5), frame, channel)
