...
_Item = tuple[int, TransitionEvent]
...
def _journal(message: str) -> TransitionEvent:
    # A fixed ts_ms keeps two constructions of the same fact equal, so tests can
    # compare a delivered event against a freshly built expectation.
...
def test_journal_self_loop_leaves_the_count_untouched() -> None:
    stream = EventStream()
    stream.emit(_conn(SessionEvent.CONNECTION_OPENED, SessionState.WAITING, SessionState.STREAMING))
    stream.emit(_conn(SessionEvent.CHUNK_READY, SessionState.STREAMING, SessionState.STREAMING))
    stream.emit(_conn(SessionEvent.COMMAND, SessionState.STREAMING, SessionState.STREAMING))
    assert stream.snapshot().connections == 1
...
async def test_subscribe_replays_events_after_since() -> None:
    stream = EventStream()
    stream.emit(_journal("a"))
    stream.emit(_journal("b"))
    stream.emit(_journal("c"))

    agen = stream.subscribe(since=1)
    assert [await _next(agen), await _next(agen)] == [(2, _journal("b")), (3, _journal("c"))]
    await _aclose(agen)


async def test_subscribe_without_since_skips_the_backlog() -> None:
    stream = EventStream()
    stream.emit(_journal("old"))
    ...
    stream.emit(_journal("new"))

    assert await pending == (2, _journal("new"))
    await _aclose(agen)
...
async def test_events_after_subscribe_arrive_before_first_iteration() -> None:
    ...
    stream.emit(_journal("between"))
    assert await _next(agen) == (1, _journal("between"))
    await _aclose(agen)
...
async def test_since_beyond_the_journal_still_delivers_live_events() -> None:
    ...
    stream.emit(_journal("b"))
    assert await _next(agen) == (2, _journal("b"))
    await _aclose(agen)
...
async def test_live_event_reaches_every_subscriber() -> None:
    ...
    stream.emit(_journal("x"))

    assert [await task for task in waiting] == [(1, _journal("x")), (1, _journal("x"))]
    await _aclose(first)
...
async def test_resume_after_a_gap_has_no_dupes_or_holes() -> None:
    stream = EventStream()
    stream.emit(_journal("1"))

    agen = stream.subscribe(since=0)
    assert await _next(agen) == (1, _journal("1"))
    await _aclose(agen)  # consumer drops

    stream.emit(_journal("2"))
    stream.emit(_journal("3"))
...
async def test_replay_is_capped_and_the_dropped_span_shows_as_a_gap() -> None:
    ...
    assert replayed == [(3, _journal("2")), (4, _journal("3"))]
    await _aclose(agen)
...
async def test_slow_subscriber_drops_its_oldest_and_stays_on_the_live_tail() -> None:
    ...
    assert tail == [(4, _journal("3")), (5, _journal("4"))]
    await _aclose(agen)
